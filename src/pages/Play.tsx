...
const SYSTEM_PROMPT = `Você é Lucas Gomes conversando com visitantes do seu portfolio.

Sobre Lucas Gomes:
- Nome completo: Lucas Gomes
- Localização: Brasil, SP
- Título: Engenheiro de Software AI-Native
- Idade: 16 anos
- Idiomas: português brasileiro e inglês fluente
- Perfil: autodidata, começou a programar em 2025
- Foco: IA, automação, desenvolvimento fullstack, WebGL e experiências interativas
- Skills: TypeScript, React, Next.js, Node.js, Python, OpenAI, Three.js, React Three Fiber, GSAP, WebGL, Vite e Clean Code
- Projetos: Bonds Agence, 3D Portfolio e Gomes Bot v2
- Contato: la181009@gmail.com

Regras:
1. Responda em português brasileiro.
2. Fale em primeira pessoa como Lucas, de forma natural e objetiva.
3. Quando perguntarem sobre projetos, use apenas os projetos listados acima.
4. Se perguntarem algo pessoal que não está aqui, responda com cuidado e direcione para tecnologia, projetos ou contato.
5. Não invente experiências, empresas, prêmios ou projetos.`;
...
const Play = () => {
  ...
  const [chatMessages, setChatMessages] = useState<ChatMessage[]>([
    { role: 'assistant', content: 'Olá! Eu sou Lucas Gomes. Pergunte qualquer coisa sobre meus projetos, skills ou ideias.' }
  ]);
  ...
  const updateGameStatus = useCallback((g: Chess) => {
    if (g.isCheckmate()) {
      setGameStatus(g.turn() === 'w' ? 'Xeque-mate! Pretas venceram.' : 'Xeque-mate! Brancas venceram.');
    } else if (g.isDraw()) {
      if (g.isStalemate()) setGameStatus('Empate por afogamento');
      else if (g.isThreefoldRepetition()) setGameStatus('Empate por repetição');
      else if (g.isInsufficientMaterial()) setGameStatus('Empate por material insuficiente');
      else setGameStatus('Empate');
    } else if (g.isCheck()) {
      setGameStatus(g.turn() === 'w' ? 'Brancas em xeque!' : 'Pretas em xeque!');
    } else {
      setGameStatus(g.turn() === 'w' ? "Vez das brancas" : "Vez das pretas");
    }
  }, []);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const errorMessage: ChatMessage = {
        role: 'assistant',
        content: 'Estou com problemas de conexão agora. Tente de novo em instantes.'
      };
      ...
      <div className="chess-container">
        ...
        <div className="chat-panel">
          <div className="chat-header">
            <span className="chat-title">Fale comigo</span>
          </div>
          ...
          <div className="player-bar opponent-bar">
            <div className="player-info">
              <div className="player-avatar">
                <img src={config.developer.profileImage} alt={config.developer.fullName} />
              </div>
              <div className="player-details">
                <span className="player-name">Lucas</span>
                <span className="player-rating">{engineThinking ? 'Pensando...' : 'Oponente'}</span>
              </div>
            </div>
          ...
          <div className="player-bar player-bar-bottom">
            <div className="player-info">
              ...
              <div className="player-details">
                ...
                <span className="player-rating">{playerColor === 'w' ? 'Brancas' : 'Pretas'}</span>
              </div>
            </div>
            <div className="captured-pieces">
        ...
        <div className="chess-side-panel right-panel">
          ...
          <div className="move-history">
            <div className="move-history-header">Lances</div>
